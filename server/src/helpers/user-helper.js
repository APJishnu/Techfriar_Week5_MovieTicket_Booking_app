...
module.exports = {
  getAllMovies: async () => {
    try {
      ...
      const uniqueMovies = Array.from(
        new Map(movies.map((movie) => [movie._id.toString(), movie])).values()
      );
  ...
  getMovieSchedule: async (movieId) => {
    try {
      const schedule = await movieSchedule
        .find({ "movies.movie": movieId })
        .populate("theatre")
  ...
  getSeatsForShowtime: async (theatreId, movieId, showDate, showTime) => {
    try {
      ...
      if (!schedule) {
        throw new Error(
          "Schedule not found for the selected theatre, movie, date, and time"
        );
      }

      // Find the movie in the schedule
      const MovieSchedule = schedule.movies.find(
        (m) => m.movie._id.toString() === movieId
      );
      if (!MovieSchedule) {
      ...
      const showDateObj = MovieSchedule.showDates.find(
        (date) => date.date === showDate
      );
      if (!showDateObj) {
      ...
      const showtimeObj = showDateObj.times.find(
  ...
  updateMovieSchedule: async ({
    ...
    try {
      ...
      const MovieSchedule = schedule.movies.find(
        (m) => m.movie.toString() === movie._id.toString()
      );
      if (!MovieSchedule) {
        throw new Error("Movie not found in the schedule.");
      }
      ...
      selectedSeats.forEach((seatNumber) => {
        ...
        if (seat) {
          seat.isBooked = true;
          seat.bookedBy = mongoose.Types.ObjectId.isValid(userId)
            ? userId
